...
export default function ServicesContent({ servicesByCategory }: ServicesContentProps) {
  // Map category names to tab IDs
  const categoryMap: Record<string, string> = {
    "Strip Wax": "stripWax",
    "Wax Packages": "stripWaxPackages",
    "Hot Wax": "hotWax",
    "Sugaring": "sugaringWax",
    "Eyes & Brows": "eyeTreatments",
    "Nails": "nails",
    "Facials": "facials",
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {activeServices.map((service, index) => (
                  <div
                    key={index}
                    className={`flex justify-between items-center py-3 px-4 rounded-lg hover:bg-stone-50 transition ${
                      service.featured ? "bg-stone-100 ring-2 ring-stone-300" : ""
                    }`}
                  >
                    <div>
                      <span
